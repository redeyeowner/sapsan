...
const getSplitedPathnameFromReuest = (request) => {
...
module.exports = class ApplicationContainer {
  ...
  getRouterIdByRequest(request) {
    ...
    const splitedPathname = getSplitedPathnameFromReuest(request);
  ...
  getActionByRequestAndId(request, routeId) {
    const splitedPath = getSplitedPathnameFromReuest(request);
  ...
  getApplication() {
    return async (req, res) => {
      ...
      try {
        ...
        const exceptionResponse = action.handleExceptions(error);
